function [u,u_hat,omega]=svmd(signal,maxAlpha,tau,tol,stopc,nrIMFs,init_omega)
...
% nrIMFs       Overwrites stopc, and fixed the number of modes to extract (added by Iris, 31-07-2025)
...
switch nargin
    case 7 % Changed by Iris to account for the additional parameter nrIMFs
        if init_omega == 0
            omega_L(1) = 0;
        else
...
while (SC2~=1)
    ...
    if nargin >=5 % checking input of the function
        ...
        if nargin == 6;
            stopc = 5; % Use the fixed nr of nrIMFs to stop
        end
